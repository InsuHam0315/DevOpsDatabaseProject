...
const buildVehicleRoutes = (routes: any[] | undefined, vehicles: Vehicle[]): VehicleRoute[] => {
  ...
  return routes
    .map((route, index) => {
      const optionName = route?.route_option_name ?? route?.route_name;
      const vehicleId =
        route?.vehicle_id ??
        route?.vehicleId ??
        route?.vehicle ??
      ...
      const uniqueId = `${vehicleId}-${optionName ?? index}`;
  ...
  results.forEach((batchResult) => {
    ...
    if (Array.isArray(optimization.results)) {
      optimization.results.forEach((summary: any) => {
        ...
        const arrayPayloads = [summary.vehicle_routes, summary.routes];
        const hasSummary = Boolean(summary?.summary);
        if (!hasSummary && Array.isArray(summary.assignments)) {
          arrayPayloads.push(summary.assignments);
        }
        arrayPayloads.filter(Array.isArray).forEach((arr: any[]) => {
          candidates.push(...arr);
        });
        if (hasSummary) {
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  setBatchResults: (results) =>
    set((state) => {
        ...
        .map((br) => {
          ...
          const totals = summaries.reduce(
            (acc: { distance: number; co2: number; time: number }, summary: any) => {
              const source = summary?.summary ?? summary;
              acc.distance += toNumber(source?.total_distance_km ?? source?.total_distance);
              if (source?.total_co2_g !== undefined) {
                acc.co2 += toNumber(source.total_co2_g, 1000);
              } else {
                acc.co2 += toNumber(source?.total_co2_kg ?? source?.total_co2);
              }
              acc.time += toNumber(source?.total_time_min ?? source?.total_time);
              return acc;
            },
      ...
      const explanationKpisRaw = parseKpisFromExplanation(
        safeResults.find((item) => typeof item.llm_explanation === 'string')?.llm_explanation ?? undefined
      );
      const explanationKpis =
        explanationKpisRaw &&
        Object.fromEntries(
          Object.entries(explanationKpisRaw).filter(
